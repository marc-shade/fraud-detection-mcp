...
class PerformanceBenchmark:
    ...
    def plot_confusion_matrix(
        self, y_test: np.ndarray, y_pred: np.ndarray, model_name: str
    ) -> Path:
        """
        ...
        output_path = (
            self.output_dir / f"confusion_matrix_{model_name.replace(' ', '_')}.png"
        )
    ...
    def plot_roc_curve(
        self, models_data: List[Tuple[str, np.ndarray, np.ndarray]]
    ) -> Path:
        """
    ...
    def run_full_benchmark(self, dataset_path: Optional[Path] = None) -> Dict[str, Any]:
        ...
        logger.info(
            f"Fraud rate - Train: {y_train.mean():.2%}, Test: {y_test.mean():.2%}"
        )
        ...
        if "isolation_forest" in models:
            metrics = self.benchmark_model(
                models["isolation_forest"], X_test, y_test, "Isolation Forest"
            )
        ...
        if "xgboost" in models:
            metrics = self.benchmark_model(models["xgboost"], X_test, y_test, "XGBoost")
        ...
        if "ensemble" in models:
            ...
            logger.info(
                f"Ensemble - Accuracy: {metrics['accuracy']:.4f}, "
                f"Precision: {metrics['precision']:.4f}, "
                f"Recall: {metrics['recall']:.4f}, "
                f"F1: {metrics['f1_score']:.4f}, "
                f"ROC-AUC: {metrics['roc_auc']:.4f}"
