...
class TestSyntheticDataIntegrationClass:
    ...
    def test_init_creates_output_dir(self, tmp_path):
        ...
        with patch.dict(
            os.environ, {"FRAUD_DETECTION_DATA_DIR": str(tmp_path / "out")}
        ):
    ...
    def test_generate_anomalous_behavioral_data(self):
        ...
        sdi = SyntheticDataIntegration()
        data = sdi._generate_anomalous_behavioral_data(
            "user_002", "high_amount_patterns"
        )
        assert isinstance(data, list)
        assert len(data) > 0
...
class TestSyntheticDataMCPRegistration:
    ...
    @pytest.mark.synthetic
    @pytest.mark.unit
    def test_total_tools_count_is_28(self):
        """28 = 19 core + 5 compliance + 4 agent commerce Tier 0 (peek + consume nonce split)."""
        import asyncio
        from server import mcp
