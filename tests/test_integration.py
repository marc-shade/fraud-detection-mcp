...
class TestCompleteWorkflows:
    ...
    def test_low_risk_transaction_workflow(self, sample_transaction_data):
        ...
        assert (
            "allow_transaction" in result["recommended_actions"]
            or "monitor_closely" in result["recommended_actions"]
        )
    ...
    def test_full_multi_modal_workflow(
        ...
        assert all(
            component in comprehensive["component_scores"]
    ...
    def test_fraud_ring_detection_workflow(self, fraud_ring_network_data):
        ...
        assert (
            "unusually_high_connectivity" in result["risk_patterns"]
            or "potential_fraud_hub" in result["risk_patterns"]
        )
    ...
    def test_escalating_risk_workflow(self):
        ...
        result2 = analyze_transaction(
            transaction, include_behavioral=True, behavioral_data=suspicious_behavioral
        )
        ...
        network_data = {
            "entity_id": "test_user",
            "connections": [
                {"entity_id": f"suspicious_{i}", "strength": 0.9} for i in range(30)
            ],
        }
    ...
    def test_batch_analysis_workflow(self):
        ...
        transactions = [
            {"amount": float(i * 100), "merchant": f"Merchant_{i}"} for i in range(10)
        ]
    ...
    def test_recommendation_consistency_workflow(self, sample_transaction_data):
        ...
        if risk_level == "CRITICAL":
            ...
            assert any(
                "review" in action or "verification" in action for action in actions
            )
        elif risk_level == "LOW":
